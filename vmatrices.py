...
def vmatrices(angle, phi_bit, psi_bit, NSUBC_VALID, Nr, Nc_users, standard, config):
    if standard == "AX":
        if config == "4x2":
        ...
        if config == "3x1":
            ...
            # v_matrix_all = np.reshape(v_matrix_all, (v_matrix_all.shape[0] * v_matrix_all.shape[1], v_matrix_all.shape[2]))
            # return v_matrix_all
